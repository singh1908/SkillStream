...
const TeacherLogin = () => {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            const {success, message, jwtTokenTeacher, name, error} = result;
            if(success){
                handleSuccess(message);
                localStorage.setItem("teacher_token", jwtTokenTeacher);
        ...
        <div className="w-full max-w-md p-8 bg-white shadow-md rounded-lg ">
            ...
            <form onSubmit={handleLogin}>
                ...
                <div className='mt-4 flex justify-center'>Don't have an account ?
                    <Link to="/teacher-signup" className='underline pl-1 font-semibold'>SignUp</Link>
                </div>
            </form>
          </div>
      </div>
