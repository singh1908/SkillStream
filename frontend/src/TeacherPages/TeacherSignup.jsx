...
const TeacherSignup = () => {
    ...
    const handleSignup = async (e) => {
        e.preventDefault();
        const {name, subject, email, password} = signupInfo;
        if(!name || !subject || !email || !password){
          return handleError("All fields are required")
        }
        try {
            const url = `http://localhost:8080/auth/teacher-signup`;
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
        <div className="w-full max-w-md p-8 bg-white shadow-md rounded-lg ">
            ...
            <form onSubmit={handleSignup}>
                ...
                <div className="mb-4">
                    <label className="block text-gray-700 mb-2" htmlFor="name">Subject <span className='text-red-600'>*</span></label>
                    <input
                        onChange={handleChange}
                        type='text'
                        name='subject'
                ...
                <div className='mt-4 flex justify-center'>Already have an account ?
                    <Link to="/teacher-login" className='underline pl-1 font-semibold'>Login</Link>
                </div>
            </form>
          </div>
      </div>
